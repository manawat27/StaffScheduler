...
export class KeycloakUserController {
  ...
  create(
    ...
    @Body() createKeycloakUserDto: CreateKeycloakUserDto & { role: AppUsersRoles; phone: string }
  ) {
  ...
  update(
    ...
    @Body() updateKeycloakUserDto: UpdateKeycloakUserDto & { role: AppUsersRoles; phone: string }
  ) {
    return this.keycloakUserService.update(uuid, updateKeycloakUserDto, user.id);
  }
